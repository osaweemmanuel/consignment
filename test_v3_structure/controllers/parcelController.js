...
const parcelCreation = async (req, res) => {
  try {
    const {
      senderName, senderGender, senderPhone, senderNationality,
      receiverName, receiverPhone, receiverEmail, receiverNationality, receiverGender,
      origin, weight, destination, destinationLatitude, destinationLongitude,
      service_type, description, parcelName, deliveryDate,
    ...
    if (req.file) {
      try {
        const uploadResponse = await cloudinary.uploader.upload(req.file.path, { folder: 'parcel_images' });
        await fs.unlink(req.file.path);
        imageUrl = uploadResponse.secure_url;
        imagePublicId = uploadResponse.public_id;
      } catch (uploadError) {
...
const updateParcelLocation = async (req, res) => {
  try {
   ...
   await db.execute(updateQuery, queryParams);
    ...
    // High Standard: Log this action in the Activity Ledger
    await logActivity(req.user.id, 'PARCEL_UPDATED', { 
      trackingNumber, 
      status, 
      currentLocation 
    });
